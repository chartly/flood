...
void ResourceTaskRun(ResourceLoadOptions* opts)
{
    ...
    if (options->sendLoadEvent)
    {
        ...
        res->resourceEvents.pushBack(event);
    }

cleanup:

    res->numResourcesQueuedLoad.store(res->numResourcesQueuedLoad.fetch_add(1));
...
void ResourceManager::destroyHandles()
{
    for( auto it = resources.begin(); it != resources.end(); ++it )
    {
        ...
        LogDebug("Resource %s (refs: %d)", res->path.c_str(),
            res->references.load());
    }
...
{
    numResourcesQueuedLoad.store(numResourcesQueuedLoad.fetch_add(1));

    ResourceTaskRun(&options);
...
void ResourceManager::removeUnusedResources()
{
    ...
    for(auto e : resources)
    {
        ...
        if( resource.Resolve()->references.load() == 1 )
