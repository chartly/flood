...
namespace dit {
    ...
    void GLApp::onLoad()
    {
        ...
        // set up the camera
        camera.position() = vec3{ 0.f, .25f, 1.f };
        camera.nearPlane() = .1f;
    ...
    void GLApp::update()
    {
        ...
        {
            // fps mouse controls
            camera.rotate(vec3{ 0.f, 1.f, 0.f }, mouseDelta.x);
            camera.rotate(vec3{ 1.f, 0.f, 0.f }, mouseDelta.y);
        ...
        if (camDebugClock.getElapsed() > 1.f)
        {
            ...
            cf.vtx = { {
                pos, pos + (.5f * camera.right()),
                pos, pos + (.5f * camera.up()),
                pos, pos + (.5f * camera.forward())
            } };
            cf.buffer.create();
    ...
    void GLApp::draw()
    {
        ...
        modelShader->uniforms[3].set(wv);
        modelShader->uniforms[4].set(wit);
        modelShader->uniforms[5].set(wvp);

        // world geometry
        glBindVertexArray(quad.buffer.id);
        glDrawArrays(GL_TRIANGLES, 0, quad.vtx.size());

        // world center coord cross
